...
if(isset($_POST["Th_Dong_y"])){
    $tenkhoa=$_POST["Th_tenkhoa"];
    if($tenkhoa!=""){
        $chuoiSQL = "INSERT INTO khoa(tenkhoa)
                    VALUES(:tenkhoa)";
        $sta = $conn->prepare($chuoiSQL);
        $sta->bindParam(':tenkhoa', $tenkhoa);
        $sta->execute();
        $thong_bao = 'Thêm khoa thành công ^-^';
    }
}
?>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card border">
                    <div class="card-body">
                        <h3 class="card-title text-success">THÊM KHOA</h3>
                        <form method="POST">
                            <div class="form-group">
                                <label for="">Nhập tên Khoa </label>
                                <input type="text" name="Th_tenkhoa" id="" class="form-control" placeholder="">
                            </div>
                            <div class="form-group text-center">
                                <button type="submit" class="btn btn-outline-danger" name="Th_Dong_y" >Đồng ý</button>
                            </div>
                        </form>
